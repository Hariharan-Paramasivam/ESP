...
void createQueue(struct Queue* q, int size) {
    q->size = size;
    q->items = (int*)malloc(size * sizeof(int));  // Dynamically allocate memory for queue items
    q->head = -1;
    q->tail = -1;
...
// Function to remove an element from the queue (Dequeue) and shift elements
void dequeue(struct Queue* q) {
    ...
    q->tail--;  // Decrement tail since we've shifted elements
